...
```
                    [CLAS-0] What Is Classification? (decision theory)
                                    │
                                    ▼
                    [CLAS-1] The Bayes-Optimal Classifier (0-1 loss, the irreducible floor)
                                    │
        ┌───────────────────┬───────────────┴───────────┬────────────────────┐
        ▼                   ▼                            ▼                    ▼
 INSTANCE-BASED       GENERATIVE                   LINEAR/MARGIN          (eval, far right)
 [CLAS-2] k-NN        [CLAS-3] Naive Bayes         [CLAS-5] The Perceptron
        │             [CLAS-4] LDA / QDA                   │
        │                   │                            ▼
        │                   │              [CLAS-6] The Margin (max-margin idea)
        │                   │                            │
        │                   │                            ▼
        │                   │              [CLAS-7] Hard-Margin SVM (the primal QP)
        │                   │                            │
        │                   │                            ▼
        │                   │              [CLAS-8] Soft-Margin SVM & the Hinge Loss
        │                   │                            │
        │                   │                            ▼
        │                   │              [CLAS-9] The Dual & Support Vectors
        │                   │                            │
        │                   │                            ▼
 ...
 TREE-BASED & ENSEMBLES (own sub-spine off CLAS-1)
...
Linear reading order: **CLAS-0 → CLAS-1**, then the three method families (instance-based **CLAS-2**; generative **CLAS-3–CLAS-4**; margin **CLAS-5–CLAS-10**) and the tree/ensemble spine (**CLAS-11–CLAS-16**) are parallel and may be read in any order once CLAS-1 is done. The convergence articles **CLAS-17–CLAS-20** assume the reader has met at least one classifier and are read last.

---

## A. The Classification Problem & Decision Theory

### CLAS-0 — What Is Classification? The Decision-Theoretic Frame
...
3. **Assumes:** *(this track)* none — entry point. *(cross-track)* joint and conditional distributions, expectation [Probability Foundations]; the idea of a regression/loss objective [Regression REGR-0].
4. **Sets up:** CLAS-1 (the optimal rule under 0-1 loss), and every downstream method (each one is an *approximation* to the rule CLAS-1 derives).
5. **Depth:** short–medium (~10–14). Motivational, light on derivation.

### CLAS-1 — The Bayes-Optimal Classifier: The Best Any Method Could Do
...
3. **Assumes:** *(this track)* CLAS-0. *(cross-track)* Bayes' rule / posterior `P(y|x)`, conditional expectation [Probability Foundations].
4. **Sets up:** the generative/discriminative split that organizes the whole track; the Bayes-error benchmark used in every evaluation article; CLAS-2 (k-NN as a nonparametric estimate of `P(y|x)`), CLAS-3–CLAS-4 (generative), CLAS-9/CLAS-8 (margin methods as boundary estimators).
...
### CLAS-2 — k-Nearest Neighbors: Classification With No Model At All
...
2. **Scope / inside:**
   - The k-NN rule as a *local, nonparametric estimate* of `P(y|x)` — the empirical class proportions in a neighborhood — and hence a direct approximation to the Bayes classifier (payoff from CLAS-1).
...
3. **Assumes:** *(this track)* CLAS-1. *(cross-track)* norms and distance/metric in `ℝᵈ`, volumes of balls [Geometry of Data]; bias–variance decomposition [Regression REGR-13].
...
### CLAS-3 — Naive Bayes: When "Pretend Features Are Independent" Wins Anyway
...
2. **Scope / inside:**
   - The generative recipe: estimate class priors `P(y)` and class-conditional `P(x|y)`, classify by `argmax_k P(y=k)·P(x|y=k)` (Bayes' rule, from CLAS-1).
...
3. **Assumes:** *(this track)* CLAS-1. *(cross-track)* Bernoulli/multinomial/Gaussian distributions, likelihood and MLE for these [Probability Foundations; MLE principle owned by Statistical Inference INFR-B1–INFR-B2 — linked, not re-derived].
4. **Sets up:** CLAS-4 (LDA is "Gaussian Naive Bayes without the independence assumption, sharing a covariance"); the spam/text running example reused in evaluation articles.
5. **Depth:** medium (~14–18).

### CLAS-4 — Discriminant Analysis: LDA, QDA, and the Gaussian Decision Boundary
...
2. **Scope / inside:**
   - Model `P(x|y=k) = N(μₖ, Σₖ)`; plug into the Bayes rule (CLAS-1) and derive the **discriminant function**. **Shared covariance** `Σₖ = Σ` ⇒ the quadratic term cancels ⇒ a **linear** boundary (**LDA**); per-class `Σₖ` ⇒ the term survives ⇒ a **quadratic** boundary (**QDA**) — both derived line by line.
...
### CLAS-5 — The Perceptron: The First Learning Machine
...
2. **Scope / inside:**
   ...
   - **Failure modes:** non-separable data (never converges), the arbitrary boundary it settles on (any separator, not the best one) — the precise gap that *motivates the margin* (CLAS-6) and the historical XOR/Minsky-Papert wall (motivates kernels/networks).
3. **Assumes:** *(this track)* CLAS-1. *(cross-track)* dot product, hyperplanes, vector norms, projection / signed distance to a plane [Geometry of Data]; (sub)gradient descent basics [Optimization].
4. **Sets up:** CLAS-6 (the perceptron finds *a* separator; the margin asks for the *best*); the linear-boundary geometry reused by every SVM article.
5. **Depth:** medium–deep (~16–22).

### CLAS-6 — The Margin: Which Separating Hyperplane Is Best?
...
3. **Assumes:** *(this track)* CLAS-5. *(cross-track)* point-to-hyperplane distance, vector norms [Geometry of Data].
4. **Sets up:** CLAS-7 (this objective *is* the hard-margin SVM); the margin concept reused in CLAS-8 (slack) and CLAS-15–CLAS-16 (boosting maximizes a margin too).
5. **Depth:** medium (~12–18).

### CLAS-7 — The Hard-Margin SVM: A Quadratic Program for the Widest Street
...
2. **Scope / inside:**
   - The **primal hard-margin program**: `min (1/2)‖w‖²` s.t. `yᵢ(wᵀxᵢ+b) ≥ 1 ∀i` — assembled directly from CLAS-6.
   ...
   - Geometric reading of the constraints; the support points that sit *on* the margin (preview of CLAS-9's support vectors).
   - **Failure mode:** hard margin requires perfect linear separability — a single noisy point makes it infeasible or pathological — which forces the soft margin (CLAS-8).
3. **Assumes:** *(this track)* CLAS-6. *(cross-track)* convex sets/functions, constrained optimization, what a QP is [Optimization].
4. **Sets up:** CLAS-8 (relax the constraints), CLAS-9 (solve via the dual).
5. **Depth:** medium–deep (~16–20).

### CLAS-8 — The Soft-Margin SVM and the Hinge Loss
...
3. **Assumes:** *(this track)* CLAS-7. *(cross-track)* convex surrogate losses, regularization as a penalty [Optimization]; cross-validation [Statistical Inference / Regression REGR-13]; **link to Regression REGR-14** for the `‖w‖²` ridge penalty parallel.
4. **Sets up:** CLAS-9 (the dual of *this* program is what's actually solved); the surrogate-loss view that connects SVMs, logistic regression, and boosting.
...
### CLAS-9 — The Dual, the KKT Conditions, and Support Vectors
...
3. **Assumes:** *(this track)* CLAS-8. *(cross-track)* Lagrange multipliers, duality, KKT conditions [Optimization]; inner products [Geometry of Data].
...
2. **Scope / inside:**
   - The trick: since the dual (CLAS-9) uses inputs *only* as inner products, replace `xᵢ·xⱼ` with `K(xᵢ,xⱼ) = φ(xᵢ)·φ(xⱼ)` for an implicit feature map `φ` — compute the inner product in the rich space without ever forming `φ(x)`.
   ...
   - **Failure modes & judgment:** RBF bandwidth as the bias–variance dial (overfitting at small bandwidth — the curse of dimensionality returns, tie to CLAS-2); kernel choice as the real modeling decision; why kernel SVMs lost ground to trees/nets on large tabular and high-`n` data.
3. **Assumes:** *(this track)* CLAS-9. *(cross-track)* inner-product/feature spaces, positive-semidefinite matrices, the Gram matrix [Geometry of Data].
...
3. **Assumes:** *(this track)* CLAS-1. *(cross-track)* entropy/log, expectation [Probability Foundations]; concavity / Jensen-flavored argument [Optimization, light].
...
3. **Assumes:** *(this track)* CLAS-11 (weak learner = stump/shallow tree), CLAS-8 (the surrogate-loss view), CLAS-12. *(cross-track)* exponential/log, weighted empirical risk [Optimization]; **additive-model framing links to Regression REGR-26 (GAMs)** as the regression cousin of stagewise fitting.
...
3. **Assumes:** *(this track)* CLAS-5–CLAS-10 (binary SVM/perceptron), and any one classifier. *(cross-track)* softmax / multinomial logit links to **Regression REGR-17–REGR-18** — not re-derived.
...
2. **Scope / inside:**
   ...
   - The **accuracy paradox** under class imbalance (the honest motivating failure — 99% accuracy by predicting the majority class); base rates; the cost-sensitive tie back to CLAS-1's asymmetric-loss Bayes rule.
   - Multi-class extension: micro vs. macro vs. weighted averaging, and which to report when.
3. **Assumes:** *(this track)* CLAS-1 (asymmetric loss), CLAS-17 (multi-class). *(cross-track)* conditional probability / rates [Probability Foundations].
...
2. **Scope / inside:**
   ...
   - The diagonal baseline, the convex hull, operating-point selection tied to the cost structure (back to CLAS-1); multi-class one-vs-rest ROC.
...
2. **Scope / inside:**
   ...
   - Why specific methods miscalibrate: Naive Bayes (independence pushes scores to extremes, from CLAS-3), SVM margins (not probabilities at all), random forests (variance shrinks scores toward the middle, from CLAS-14) — each explained mechanistically.
...
3. **Assumes:** *(this track)* CLAS-19, and the methods CLAS-3/CLAS-14 it diagnoses. *(cross-track)* logistic transform links to **Regression REGR-17**; isotonic / monotone fitting [Optimization, light].
...
- *Joint / conditional distributions, conditional expectation, Bayes' rule for `P(y|x)`* — needed at **CLAS-0, CLAS-1**.
- *Bernoulli, multinomial, Gaussian (univariate & multivariate) distributions; their likelihoods* — needed at **CLAS-3, CLAS-4**.
...
- *Norms, distance/metric in `ℝᵈ`, volumes of balls* — needed at **CLAS-2** (k-NN, curse of dimensionality).
- *Dot product, hyperplanes, point-to-plane signed distance, projection* — needed at **CLAS-5, CLAS-6**.
- *Quadratic forms, eigenvalues, the generalized eigenproblem, matrix inverse* — needed at **CLAS-4** (LDA/QDA, Fisher's discriminant).
- *Inner-product / feature spaces, positive-semidefinite matrices, the Gram matrix* — needed at **CLAS-9, CLAS-10** (the dual and the kernel trick).

**From Optimization**
- *(Sub)gradient descent* — needed at **CLAS-5** (perceptron), **CLAS-16** (functional gradient descent).
- *Convex sets/functions, constrained optimization, quadratic programs* — needed at **CLAS-7** (the SVM primal).
- *Lagrangian duality, KKT conditions, complementary slackness* — needed at **CLAS-9** (the SVM dual).
- *Convex surrogate losses, regularization as a penalty* — needed at **CLAS-8** (hinge loss).
...
- *Logistic regression, the logit/sigmoid, the Bernoulli likelihood, IRLS* (**REGR-17**) — linked at **CLAS-4** (discriminative contrast), **CLAS-16, CLAS-20** (log-loss / Platt scaling), **CLAS-17** (softmax).
...
- *Ridge / `ℓ₂` penalty* (**REGR-14**) — linked at **CLAS-8** (the `‖w‖²` regularizer parallel).
- *Bias–variance decomposition & cross-validation* (**REGR-13**) — needed at **CLAS-2, CLAS-8, CLAS-12, CLAS-16**.

**From Statistical Inference** *(link, do NOT re-derive — Inference owns the general machinery)*
- *MLE as a general estimation principle* (**INFR-B1–INFR-B2**) — linked at **CLAS-3, CLAS-4** (fitting the generative models).
...
|---|---|---|
| **Logistic regression, the logit, GLMs, exponential family, IRLS** | **Regression (REGR-17–REGR-19)** | Already owned and derived there. Classification links for the discriminative contrast (CLAS-4), softmax (CLAS-17), log-loss boosting (CLAS-16), and Platt scaling (CLAS-20) — never re-derives. |
...
| **The Bayes classifier & statistical decision theory (0-1 loss, risk)** | **Classification (CLAS-0–CLAS-1)** | This is the foundational frame for *classification specifically*. If a future general **Decision Theory** stub is wanted, it could lift CLAS-1 — but it belongs here by default. |
...
| **Curse of dimensionality, distance geometry, kernels/feature maps, PSD matrices** | **Geometry of Data** | Classification consumes (CLAS-2, CLAS-10). Kernel PCA / spectral methods belong to Unsupervised Learning, which can reuse CLAS-10's kernel-trick build. |
| **Convex optimization, duality/KKT, gradient & coordinate methods** | **Optimization** | Classification consumes heavily (CLAS-5, CLAS-7–CLAS-9, CLAS-16). The SVM dual is the marquee *application*; the duality *theory* is Optimization's. |
| **LDA as supervised dimensionality reduction** | **Shared with Unsupervised Learning / Geometry of Data** | The classifier derivation is here (CLAS-4); the dimensionality-reduction *use* (and its unsupervised PCA sibling) is better owned by Unsupervised Learning, linking back to CLAS-4. |
| **Cost-sensitive / asymmetric-loss decisions; calibrated probabilities for actions** | **Link to Causal Inference where decisions are the deliverable** | CLAS-1 and CLAS-20 supply the loss/calibration machinery; how a *decision* is taken from a probability under intervention is Causal Inference's concern. |
...
- **Decision theory comes first (CLAS-0–CLAS-1) on purpose.** Every subsequent method is framed as an *approximation to the Bayes classifier* — k-NN estimates `P(y|x)` locally, generative models estimate `P(x|y)` then invert, margin methods estimate the boundary directly. The Bayes-error floor gives every evaluation article a benchmark. Without this spine the track is a disconnected zoo of algorithms.
- **The generative/discriminative axis is the organizing contrast,** introduced at CLAS-1 and paid off repeatedly (CLAS-3–CLAS-4 generative vs. linked logistic regression discriminative; CLAS-16's log-loss boosting as discriminative).
- **The SVM is built as a six-article staircase (CLAS-5–CLAS-10)** — perceptron → margin → primal → soft margin/hinge → dual → kernel — because each step answers exactly one failure of the previous, and the inner-product structure at CLAS-9 is what *earns* the kernel trick at CLAS-10. Collapsing these would violate inch-wide.
...
This roadmap is **21 articles** (CLAS-0–CLAS-20): 2 foundation, 1 instance-based, 2 generative, 6 margin-based, 6 tree/ensemble, 4 strategy/evaluation. Several deep articles (CLAS-4, CLAS-8, CLAS-9, CLAS-10, CLAS-11, CLAS-15, CLAS-16) carry pre-marked split seams should they push the 30-page ceiling during drafting.
