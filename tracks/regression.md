...
```
   ...
   (t, F, confidence/prediction          │                          │
    intervals, hypothesis tests)         │                          │
              ...
              └──────────────┬───────────┴──────────────────────────┘
                             ▼
              [REGR-10] Diagnostics & Leverage
              (residual analysis, influence, outliers)
                             │
              ┌──────────────┼───────────────────────────────┐
              ▼              ▼                                 ▼
  [REGR-11] When Variance     [REGR-12] Maximum-Likelihood       [REGR-13] Bias–Variance &
  ...
  │            │                                            │
  │            ▼                                            │
  │ [REGR-15] Lasso & the Geometry of Sparsity                  │
  │            │                                            │
  │            ▼                                            │
  │ [REGR-16] Elastic Net & the Regularization Path             │
  └─────────────────────────────────────────────────────────┘
        │
        │   ┌──────────────────── GLM SUB-BRANCH ──────────────────┐
        │   │ [REGR-17] Logistic Regression (regression for a yes/no)   │
        │   │            │                                          │
        │   │            ▼                                          │
        │   │ [REGR-18] The GLM Framework (link + exponential family)   │
        │   │            │                                          │
        │   │            ▼                                          │
        │   │ [REGR-19] Poisson & Count Regression                      │
        │   └────────────────────────────────────────────────────┘
        │
        ▼
  ┌──────────────── NONPARAMETRIC / FLEXIBLE SUB-BRANCH ───────────────┐
  │ [REGR-20] Polynomial & Basis-Function Regression (still linear in β)     │
  │            │                                                         │
  │            ▼                                                         │
  │ [REGR-21] Regression Splines (piecewise polynomials, knots)             │
  │            │                                                         │
  │            ▼                                                         │
  │ [REGR-22] Smoothing Splines (the roughness penalty)                     │
  │            │                                                         │
  │            ▼                                                         │
  │ [REGR-23] Kernel Regression (Nadaraya–Watson: a local weighted mean)    │
  │            │                                                         │
  │            ▼                                                         │
  │ [REGR-24] Local Regression (LOESS / local polynomials)                  │
  │            │                                                         │
  │            ▼                                                         │
  │ [REGR-25] The Bias–Variance Trade-off of the Bandwidth (smoothing param)│
  │            │                                                         │
  │            ▼                                                         │
  │ [REGR-26] Generalized Additive Models (GAMs: sum of smooths)            │
  └─────────────────────────────────────────────────────────────────────┘
```
...
### REGR-10 — Diagnostics, Leverage, and Influence
...
### REGR-11 — When Variance Isn't Constant: WLS, GLS, and Robust Standard Errors
...
### REGR-12 — The Maximum-Likelihood View of Regression
...
### REGR-13 — Bias, Variance, and Choosing a Model
...
### REGR-15 — Lasso and the Geometry of Sparsity
...
3. **Assumes:** REGR-14; **cross-track:** *convex functions and subgradients*, *coordinate descent* [Optimization track].
4. **Sets up:** REGR-16.
...
### REGR-16 — Elastic Net and the Regularization Path
...
3. **Assumes:** REGR-12 (MLE view), REGR-11 (reweighting); **cross-track:** *Bernoulli/binomial distribution* [Probability]; *Newton's method / gradient methods* [Optimization].
4. **Sets up:** REGR-18.
...
### REGR-18 — The GLM Framework: Link Functions and the Exponential Family
...
2. **Scope / inside:**
   ...
   - The canonical link and why it simplifies the score equations; IRLS as the unifying fitting algorithm (WLS-in-a-loop, payoff from REGR-11).
   - Deviance as the GLM generalization of SSE; deviance-based model comparison.
3. **Assumes:** REGR-11, REGR-12, REGR-17; **cross-track:** *exponential family of distributions* [Probability / Estimation track].
4. **Sets up:** REGR-19; REGR-26 (GAMs extend GLMs with smooths).
5. **Depth:** deep (~20–30).

### REGR-19 — Poisson and Count Regression
...
3. **Assumes:** REGR-18; **cross-track:** *Poisson and negative-binomial distributions* [Probability track].
4. **Sets up:** (Terminal in GLM branch; feeds REGR-26.)
5. **Depth:** medium (~12–20).
...
### REGR-21 — Regression Splines: Piecewise Polynomials with Knots
...
### REGR-23 — Kernel Regression: A Local Weighted Mean (Nadaraya–Watson)
...
### REGR-24 — Local Regression: LOESS and Local Polynomials
...
2. **Scope / inside:**
   - Local linear/quadratic regression; the weighted-least-squares solve at each target point (direct reuse of REGR-11's WLS).
...
3. **Assumes:** REGR-11, REGR-23.
4. **Sets up:** REGR-25.
5. **Depth:** deep (~20–30).

### REGR-25 — The Bandwidth Is the Model: Bias-Variance of Smoothing
...
3. **Assumes:** REGR-13, REGR-22, REGR-23, REGR-24.
4. **Sets up:** REGR-26 (motivates additive structure as the escape from the curse).
5. **Depth:** deep (~20–30).

### REGR-26 — Generalized Additive Models: A Sum of Smooths
...
- *Convex functions and subgradients* — needed at **REGR-15** (lasso).
- *Coordinate descent* — needed at **REGR-15**.
- *Newton's method / gradient methods* — needed at **REGR-17** (logistic fitting).
- *Calculus of variations (gentle)* — needed at **REGR-22** (smoothing splines); partially built inline since it's the one above-floor calculus touch.
