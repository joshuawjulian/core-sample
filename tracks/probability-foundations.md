...
```
                 [PROB-00] What Is Probability? (sample spaces & the axioms)
                                     │
                                     ▼
                 [PROB-01] Conditional Probability, Independence & Bayes' Rule
                                     │
                                     ▼
                 [PROB-02] Random Variables: Turning Outcomes into Numbers
                                     │
                  ┌──────────────────┼───────────────────┐
                  ▼                                       ▼
   [PROB-03] Discrete RVs (pmf, CDF)                [PROB-04] Continuous RVs (pdf, CDF)
                  └──────────────────┬───────────────────┘
                                     ▼
                 [PROB-05] Expectation: The Long-Run Average
                                     │
                                     ▼
                 [PROB-06] Variance, Standard Deviation & Variance-as-Sum-of-Squares
                                     │
                                     ▼
                 [PROB-07] Joint Distributions, Covariance & Correlation
                                     │
                  ┌──────────────────┼───────────────────┐
                  ▼                                       ▼
   [PROB-08] Independence & the i.i.d. Sample       [PROB-09] Conditional Distributions
...
The catalog (PROB-10–PROB-16) hangs off the joint/conditional foundation (PROB-05–PROB-09) and can be read in parallel with the vector/limit spine (PROB-17–PROB-23) once expectation and variance exist. See the reading-order note at the end.

---

## Movement 1 — The ground floor: events, conditioning, random variables

### PROB-00 — What Is Probability? Sample Spaces and the Axioms
...
4. **Sets up:** PROB-01 (conditioning), PROB-02 (a random variable is a function on Ω). The true entry point of the whole series.
5. **Depth:** medium (~12–16). Motivational and definitional.

### PROB-01 — Conditional Probability, Independence, and Bayes' Rule
...
3. **Assumes:** PROB-00.
4. **Sets up:** PROB-08 (independence of random variables), PROB-09 (conditional distributions), PROB-12 (Poisson via independence). **Cross-track payoff:** Statistical Inference D1 & E4, Causal Inference, Classification (naive Bayes) all consume this.
...
2. **Scope / inside:**
   ...
   - Discrete vs. continuous vs. mixed previewed via the CDF's jumps and flats — motivating the PROB-03/PROB-04 split.
3. **Assumes:** PROB-00, PROB-01.
4. **Sets up:** PROB-03, PROB-04 (the two concrete kinds), PROB-22 (the *empirical* CDF is this object estimated from data).
5. **Depth:** medium (~12–16).

### PROB-03 — Discrete Random Variables: The Probability Mass Function
...
3. **Assumes:** PROB-02.
4. **Sets up:** PROB-05 (expectation as a weighted sum), PROB-11/PROB-12 (the named discrete distributions).
5. **Depth:** short–medium (~10–14).

### PROB-04 — Continuous Random Variables: The Probability Density Function
...
3. **Assumes:** PROB-02; floor-level integration and the chain rule (used explicitly, not assumed away).
4. **Sets up:** PROB-05, PROB-13/PROB-14 (the named continuous distributions), PROB-18 (vector change of variables).
5. **Depth:** medium (~12–16).
...
3. **Assumes:** PROB-03, PROB-04.
4. **Sets up:** PROB-06 (variance is an expectation), PROB-07 (covariance is an expectation), every distribution's mean in the catalog, PROB-19 (Markov/Chebyshev).
5. **Depth:** medium (~12–16).

### PROB-06 — Variance, Standard Deviation, and Variance as a Sum of Squares
...
3. **Assumes:** PROB-05.
4. **Sets up:** PROB-07 (covariance generalizes this to pairs), PROB-17 (covariance matrix is the vector version), PROB-19 (Chebyshev uses variance). **Cross-track payoff:** Regression R3 (variance as sum of squares), R2.
5. **Depth:** medium (~12–16).

### PROB-07 — Joint Distributions, Covariance, and Correlation
...
3. **Assumes:** PROB-05, PROB-06.
4. **Sets up:** PROB-08 (independence), PROB-17 (covariance matrix), PROB-18 (variance of linear combinations). **Cross-track payoff:** Regression R2/R6, Geometry of Data (covariance as Gram matrix), Unsupervised Learning (PCA).
5. **Depth:** medium–deep (~14–18).

### PROB-08 — Independence and the i.i.d. Sample
...
2. **Scope / inside:**
   - Independence of random variables via factoring joint = product of marginals; independence ⇒ zero covariance (and the converse failure, tied to PROB-07).
...
### PROB-09 — Conditional Distributions and Conditional Expectation
...
2. **Scope / inside:**
   - Conditional pmf/pdf `f(y|x)=f(x,y)/f(x)`; the conditional CDF; rebuilding PROB-01's conditioning at the random-variable level.
...
3. **Assumes:** PROB-05, PROB-07, PROB-01.
...
5. **Depth:** deep (~18–24). **Split seam if needed:** cut after the two laws of total expectation/variance into *PROB-09a "Conditional Distributions & Conditional Expectation"* and *PROB-09b "The Laws of Total Expectation/Variance & Best Prediction,"* at the boundary where `E[Y|X]` becomes a predictor.
...
3. **Assumes:** PROB-00 (equally-likely events).
...
2. **Scope / inside:**
   ...
   - **Binomial(n,p)** as a sum of i.i.d. Bernoullis: pmf via PROB-10's count, mean `np` and variance `np(1−p)` via PROB-08's sum rules — derived two ways.
...
3. **Assumes:** PROB-05, PROB-06, PROB-08, PROB-10.
...
3. **Assumes:** PROB-11; PROB-05/PROB-06 (mean & variance); limits at the floor's level.
...
3. **Assumes:** PROB-04, PROB-05, PROB-06, PROB-08; the Gaussian-integral derivation flags its one above-floor step and builds it.
...
3. **Assumes:** PROB-04, PROB-05, PROB-06, PROB-12 (Poisson-process link).
...
3. **Assumes:** PROB-13 (normal), PROB-14 (gamma), PROB-08 (independence of the numerator/denominator pieces).
...
3. **Assumes:** PROB-11, PROB-12, PROB-13, PROB-14; PROB-05/PROB-06 (moments).
...
2. **Scope / inside:**
   - A **random vector** `X=(X₁,…,X_d)`; its **mean vector** `μ=E[X]` (component-wise); the **covariance matrix** `Σ` with variances on the diagonal and covariances off it (built directly from PROB-07).
...
3. **Assumes:** PROB-07, PROB-13; vectors/matrices used at a notational level, with the linear-algebra machinery flagged as cross-track (Geometry of Data).
...
3. **Assumes:** PROB-05, PROB-06, PROB-08.
...
3. **Assumes:** PROB-05, PROB-08, PROB-11–PROB-14 (catalog to compute MGFs of); complex exponentials introduced gently for the characteristic function.
...
2. **Scope / inside:**
   - The **empirical CDF** `F̂ₙ(x)=(1/n)Σ 1{Xᵢ≤x}` as the data-driven stand-in for `F` (the PROB-02 object, estimated); each `F̂ₙ(x)` as a scaled binomial, hence unbiased with variance `F(1−F)/n`, derived.
...
3. **Assumes:** PROB-02, PROB-08, PROB-11 (the indicator-as-Bernoulli view), PROB-21 (the convergence statement).
...
3. **Assumes:** PROB-10 (counting), PROB-04 (densities), PROB-14 (Beta link).
...
- *Single-variable differentiation/integration, the fundamental theorem, substitution, limits* — used throughout (PROB-04, PROB-05, PROB-12, PROB-14, PROB-20). These are *at* the floor and therefore assumable.
- *The one double-integral (Gaussian integral, polar coordinates)* at **PROB-13** and the *one Jacobian change-of-variables* at **PROB-04/PROB-18** are above the floor and are **built inline and flagged**, not assumed.
...
| Conditional probability & Bayes' **for events** | **Probability** (PROB-01) | Inference E4 / Causal / Classification consume it; **Bayes for *parameters*** (posterior ∝ likelihood × prior) is **Inference's** (E4) — different altitude, written there. |
| Conditional **expectation** `E[Y|X]` and "best predictor" | **Probability** (PROB-09) | Built here; **Regression** consumes it (R0/R23) as the thing it estimates, and **Causal Inference** consumes it for conditioning — neither re-derives it. |
...
| **Variance as a sum of squares** | **Probability** (PROB-06) | Built here exactly as Regression R3 requests; Regression uses it in the SST=SSR+SSE decomposition. |
...
- **PROB-00–PROB-02 are deliberately unglamorous** — sample spaces, conditioning, and the CDF — because every track in the series silently assumes them. Putting them at the true entry point is what lets every other track honor the Calc-1 floor.
- **Conditional expectation (PROB-09) is placed as a hinge, not a footnote.** `E[Y|X]` as the best MSE predictor is the single result that makes Regression's entire "we are estimating `E[Y|X]`" framing rigorous rather than asserted — so it gets a full, possibly split, article.
...
This roadmap is **24 articles** (PROB-00–PROB-23) in five movements. Several deep articles (PROB-09, PROB-13, PROB-14, PROB-15, PROB-17, PROB-20, PROB-21) carry pre-marked split seams should they push the 30-page ceiling during drafting. Every probability prerequisite requested by the existing Regression and Statistical Inference tracks is supplied by exactly one article here.
