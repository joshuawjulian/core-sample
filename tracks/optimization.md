...
```
                    [OPTM-0] What Is an Optimization Problem?
                    (objective, argmin, feasible set — the vocabulary)
                                     │
                                     ▼
                    [OPTM-1] 1-D Minimization: the Derivative Test
                    (f'(x)=0, second-derivative test, the Calc-1 floor)
                                     │
                                     ▼
                    [OPTM-2] Minimizing in Many Variables: ∇f = 0
                    (stationary points, the Hessian test)   ◄── needs Geometry track
                                     │
              ┌──────────────────────┼───────────────────────────┐
              ▼                                                   ▼
  ┌──────── CONVEXITY SPINE ────────┐                  ┌──── DESCENT SPINE ─────┐
  │ [OPTM-3] Convex Sets               │                  │ [OPTM-6] Gradient Descent   │
  │        │                        │                  │   (the core algorithm)  │
  │        ▼                        │                  │        │                │
  │ [OPTM-4] Convex Functions          │                  │        ▼                │
  │   (and why convexity is the     │                  │ [OPTM-7] Step Size & the    │
  │    dividing line of the field)  │                  │   Convergence of GD     │
  │        │                        │                  │   (L-smoothness, rates) │
  │        ▼                        │                  │        │                │
  │ [OPTM-5] Subgradients              │                  │        ▼                │
  │   (calculus for kinks: |x|)     │                  │ [OPTM-8] Momentum &         │
  └─────────────────────────────────┘                  │   Accelerated GD        │
              │           │                             │        │                │
              │           │                             │        ▼                │
              │           │                             │ [OPTM-9] Newton's Method &  │
              │           │                             │   2nd-Order Methods     │
              │           │                             │   (incl. Gauss–Newton,  │
              │           │                             │    quasi-Newton/BFGS)   │
              │           │                             └────────────────────────┘
...
Reading order is top-to-bottom, but the **Convexity spine (OPTM-3–OPTM-5)** and the **Descent spine (OPTM-6–OPTM-9)** are genuinely parallel after OPTM-2 and can be read in either order; they reconverge at the algorithm articles OPTM-10–OPTM-12. See the branching note at the end.
...
4. **Sets up:** OPTM-1 (the 1-D solution method), and the vocabulary every later article uses.
5. **Depth:** short (~8–12). Motivational, almost no derivation.

### OPTM-1 — One-Dimensional Minimization: the Derivative Test
...
3. **Assumes:** OPTM-0; floor-level differentiation (Calc-1 — used directly, not built).
4. **Sets up:** OPTM-2 (the same logic in many variables).
5. **Depth:** short–medium (~10–16).

### OPTM-2 — Minimizing in Many Variables: Setting the Gradient to Zero
...
3. **Assumes:** OPTM-1; **cross-track (heavy):** *the gradient `∇f`, directional derivatives, the Hessian, quadratic forms `xᵀAx`, and positive/negative-definiteness* [Geometry of Data / Applied Linear Algebra track] — used, not re-derived.
4. **Sets up:** the Convexity spine (OPTM-3–OPTM-5, which explains *when* a stationary point is the global min) and the Descent spine (OPTM-6, *how* to reach one without a closed form).
...
3. **Assumes:** OPTM-0; **cross-track:** *vectors, linear combinations, half-spaces* [Geometry of Data track].
4. **Sets up:** OPTM-4 (a convex function is one whose epigraph is a convex set), OPTM-13–OPTM-15 (constraint sets), OPTM-16 (projection onto a convex set).
5. **Depth:** medium (~12–16).

### OPTM-4 — Convex Functions: Why Convexity Is the Whole Ballgame
...
2. **Scope / inside:**
   ...
   - **Strict vs. strong convexity** (unique minimizer; quadratic lower bound) and why strong convexity is what makes gradient descent fast (forward ref to OPTM-7).
...
3. **Assumes:** OPTM-2, OPTM-3; **cross-track:** *Hessian and positive-semidefiniteness* [Geometry of Data track].
4. **Sets up:** OPTM-5 (subgradients extend this to non-smooth convex functions), OPTM-7 (convergence guarantees assume convexity), OPTM-13–OPTM-15 (convex constrained problems are the well-behaved case).
...
### OPTM-5 — Subgradients: Calculus for Functions with Kinks
...
2. **Scope / inside:**
   - The **subgradient** and **subdifferential** `∂f(x)` defined from the tangent-below inequality of OPTM-4; the picture of "all the lines that stay below the graph at a kink."
...
3. **Assumes:** OPTM-4; **cross-track:** none new beyond OPTM-2's gradient.
...
### OPTM-6 — Gradient Descent: Walking Downhill
...
3. **Assumes:** OPTM-2; **cross-track:** *gradient, Taylor's theorem (first order)* [Geometry of Data track / Calc-1 floor].
4. **Sets up:** OPTM-7 (when and how fast it converges), OPTM-8 (accelerating it), OPTM-11 (the stochastic version). **Supplies** the *gradient descent / gradient ascent* prerequisite for Regression and Inference.
...
### OPTM-7 — Step Size and Convergence: Why (and How Fast) Gradient Descent Works
...
2. **Scope / inside:**
   ...
   - **Honest boundary:** non-convex objectives (only stationarity is guaranteed); pathological curvature (zig-zagging in a narrow valley) that motivates momentum (OPTM-8) and second-order methods (OPTM-9).
3. **Assumes:** OPTM-4 (convexity, strong convexity), OPTM-6; **cross-track:** *eigenvalues / condition number* [Geometry of Data track].
4. **Sets up:** OPTM-8 (momentum attacks the `κ` dependence), OPTM-9 (Newton makes it `κ`-independent), OPTM-11 (SGD's noisier rate).
...
### OPTM-8 — Momentum and Accelerated Gradient Descent
...
2. **Scope / inside:**
   - **Heavy-ball / classical momentum**: the update with a velocity term; the rolling-ball intuition and why it damps the zig-zag of OPTM-7.
...
3. **Assumes:** OPTM-2 (Hessian test), OPTM-6, OPTM-7; **cross-track:** *Hessian, matrix inverse, positive-definiteness, solving a linear system* [Geometry of Data track].
...
2. **Scope / inside:**
   ...
   - **The lasso payoff:** with the non-smooth `ℓ₁` penalty, each coordinate update is **soft-thresholding** (derived in OPTM-5) — making coordinate descent the standard lasso/elastic-net solver. This is the exact article Regression R15 links here for.
   - Why it shines on separable penalties and sparse high-dimensional problems; the honest comparison to GD.
3. **Assumes:** OPTM-5 (subgradient / soft-thresholding), OPTM-6; **cross-track:** none new.
...
2. **Scope / inside:**
   ...
   - Why SGD needs a **decaying step size** `Σηₖ=∞, Σηₖ²<∞`; the noisy `O(1/k)` (convex) / `O(1/√k)` rate and why it plateaus at a noise floor; the role of saddle-point escape in non-convex problems (tie to OPTM-2).
...
3. **Assumes:** OPTM-6, OPTM-7; **cross-track:** *expectation, unbiased estimator, variance of an average* [Probability foundations track].
...
3. **Assumes:** OPTM-8 (momentum), OPTM-11 (SGD).
...
3. **Assumes:** OPTM-2; **cross-track:** *gradient, level sets / the gradient ⟂ level set fact* [Geometry of Data track].
...
3. **Assumes:** OPTM-4 (convexity), OPTM-13; **cross-track:** none new.
...
2. **Scope / inside:**
   ...
   - **Strong duality** and **Slater's condition** for convex problems (gap closes) — stated with its geometric (separating-hyperplane, OPTM-3) justification.
...
3. **Assumes:** OPTM-3 (separating hyperplane), OPTM-13, OPTM-14.
...
2. **Scope / inside:**
   - **Projected gradient descent**: take a gradient step, then project back onto the feasible convex set (OPTM-3); the projection defined and computed for a box / ball / simplex.
   - The **proximal operator** defined; **proximal gradient descent (ISTA)** for "smooth + non-smooth" objectives, derived; recognizing that the `ℓ₁` prox **is** soft-thresholding (full circle to OPTM-5/OPTM-10) — this is the modern lasso solver, and **FISTA** as its accelerated (OPTM-8) form.
   - Why prox/projection generalizes both coordinate descent and constrained GD; when to reach for it.
3. **Assumes:** OPTM-3, OPTM-5, OPTM-6, OPTM-8, OPTM-14.
...
3. **Assumes:** OPTM-0, OPTM-1 (the "set the derivative to zero" template it generalizes); **cross-track:** *integration by parts* [Calc-1 floor — reviewed inline].
...
- *Vectors, linear combinations, half-spaces / hyperplanes* — needed at **OPTM-3**.
- *The gradient `∇f` and directional derivatives* — needed at **OPTM-2** (and used everywhere after).
- *The Hessian and the second-order Taylor expansion* — needed at **OPTM-2, OPTM-4, OPTM-9**.
- *Quadratic forms `xᵀAx` and positive-/negative-(semi)definiteness* — needed at **OPTM-2, OPTM-4, OPTM-9**.
- *Eigenvalues and the condition number of a matrix* — needed at **OPTM-7** (convergence rate), **OPTM-9**.
- *Matrix inverse and solving a linear system* — needed at **OPTM-9** (Newton step).
...
- *Single-variable differentiation, the second-derivative test* — **OPTM-1**.
- *First-order Taylor expansion* — **OPTM-6**.
...
|---|---|---|
| **Regression R15** (lasso) | *Convex functions and subgradients* | **OPTM-4** (convex functions) + **OPTM-5** (subgradients) |
| **Regression R15** (lasso) | *Coordinate descent* | **OPTM-10** (with soft-thresholding from **OPTM-5**) |
| **Regression R17** (logistic) | *Newton's method / gradient methods* | **OPTM-6** (gradient) + **OPTM-9** (Newton / IRLS) |
| **Regression R22** (smoothing splines) | *Calculus of variations (gentle)* | **OPTM-17** |
| **Inference B2** (MLE, no closed form) | *Newton's method / gradient ascent* | **OPTM-6** (ascent) + **OPTM-9** (Newton) |
| **Inference B4** (MLE asymptotics) | *Newton's method / gradient ascent* | **OPTM-6**, **OPTM-9** |

Additional concepts this track owns and exports to tracks **currently being designed** (anticipated, see Overlaps below):
- *Gradient descent / SGD / Adam* (**OPTM-6, OPTM-11, OPTM-12**) → the training engine for **Classification** and **Unsupervised Learning** (e.g. k-means as alternating minimization, deep models).
...
| **Minimizing a loss to FIT a model** (OLS normal equations, logistic loss) | **Regression** owns the *model and loss*; **Optimization** owns the *minimization method* | OPTM-2 uses OLS as an *example* of `∇f=0`; Regression owns the regression interpretation and links here for the descent algorithm when there's no closed form. |
...
- **OPTM-0–OPTM-2 are the on-ramp, deliberately gentle.** The track starts exactly at the Calc-1 floor ("set `f'(x)=0`") so a rusty reader is never cold-started, then climbs to `∇f=0` only after the vocabulary is solid.
- **Convexity (OPTM-3–OPTM-5) and descent (OPTM-6–OPTM-9) are parallel on purpose.** Convexity answers *"is a stationary point the answer?"*; descent answers *"how do I reach a stationary point?"*. They are independent ideas that the reader should hold separately, then fuse at OPTM-7 (convergence *needs* convexity) and the algorithm articles.
- **Subgradients (OPTM-5) are placed inside the convexity spine, not bolted onto lasso.** They are a general extension of the derivative, so they're taught generally and then *cashed out* three times: OPTM-10 (coordinate descent), OPTM-16 (proximal), and Regression's lasso.
- **Newton (OPTM-9) carries Gauss–Newton/IRLS explicitly** because that is the precise link Regression's GLM articles and Inference's MLE articles request — it is cheaper to own it once here than to re-derive IRLS in two tracks.
...
This roadmap is **18 articles** (OPTM-0–OPTM-17). The two spines (convexity OPTM-3–OPTM-5, descent OPTM-6–OPTM-9) are ~3–4 articles each; constrained optimization (OPTM-13–OPTM-16) is 4; the rest are the on-ramp and the variational coda. Several deep articles (OPTM-4, OPTM-6, OPTM-7, OPTM-9, OPTM-11, OPTM-14, OPTM-15, OPTM-16, OPTM-17) carry pre-marked split seams should they push the 30-page ceiling during drafting.
