...
#### A1. **The Estimator as a Random Variable**
...
4. **Sets up:** A2 (the two limit theorems), B1 (likelihood), C1 (CIs need a sampling distribution).
5. **Depth:** medium (~12–16).

#### A2. **Two Theorems That Make Statistics Possible: LLN and the CLT**
...
3. **Assumes:** *(this track)* A1. *(cross-track)* expectation/variance, Chebyshev's inequality, moment generating functions or characteristic-function sketch [Probability foundations]; convergence concepts [Probability foundations].
4. **Sets up:** C1/C2 (CLT-based intervals), B4 (asymptotic normality of the MLE reuses CLT machinery), D2 (test statistics' null distributions).
5. **Depth:** deep (~20–26). **Split seam if needed:** cut after the LLN into *A2a "The Law of Large Numbers"* and *A2b "The Central Limit Theorem,"* at the boundary between the two convergence modes.

### B. The Likelihood Spine

#### B1. **The Likelihood: Reading Probability Backwards**
...
3. **Assumes:** *(this track)* A1. *(cross-track)* pmf/pdf, joint density of i.i.d. data, log/derivative rules [Probability foundations; Calc-1 floor].
4. **Sets up:** B2 (MLE maximizes this), B3 (Fisher information is its curvature), E4 (Bayesian posterior = likelihood × prior).
5. **Depth:** medium (~12–16).

#### B2. **Maximum Likelihood: The Parameter That Best Explains the Data**
...
3. **Assumes:** *(this track)* B1. *(cross-track)* finding maxima via derivatives, multivariable critical points [Calc-1 floor + gradients from Geometry of Data / Optimization]; Newton's method / gradient ascent [Optimization].
4. **Sets up:** B3 & B4 (properties and uncertainty of the MLE), C3 (likelihood-based intervals), D3 (likelihood-ratio test), and is consumed by all of Regression.
5. **Depth:** deep (~18–24). **Split seam:** if numerical optimization grows, cut after the closed-form section into *B2a "Maximum Likelihood (closed-form)"* and *B2b "Computing the MLE When There's No Formula,"* at the score-equation/no-closed-form boundary.

#### B3. **Fisher Information: How Much a Sample Can Tell You**
...
3. **Assumes:** *(this track)* B1, B2. *(cross-track)* expectation, variance, differentiating under the integral (stated and motivated, not assumed) [Probability foundations]; second derivatives/curvature [Calc-1 floor].
4. **Sets up:** B4 (the asymptotic variance of the MLE *is* `1/I`), C3 (Wald intervals use it), D3 (the score test uses it).
5. **Depth:** medium–deep (~16–20).

#### B4. **Why Maximum Likelihood Works: Consistency, Efficiency, and Asymptotic Normality**
...
3. **Assumes:** *(this track)* A2 (CLT), B2 (MLE), B3 (Fisher info). *(cross-track)* Taylor expansion [Calc-1 floor], convergence in distribution [Probability foundations].
4. **Sets up:** C3 (Wald CI), D3 (the trinity of asymptotic tests), and the asymptotic inference all of Regression relies on.
5. **Depth:** deep (~22–28). **Split seam:** cut after asymptotic normality into *B4a "Consistency & Asymptotic Normality of the MLE"* and *B4b "Efficiency and the Delta Method,"* at the point where the limiting distribution is established.

### C. Interval Estimation

#### C1. **Confidence Intervals: What "95% Confident" Actually Means**
...
3. **Assumes:** *(this track)* A1, A2. *(cross-track)* Normal quantiles [Probability foundations].
4. **Sets up:** C2 (t-interval), C3 (likelihood/Wald intervals), E2 (bootstrap CI), and contrasts with E4 (Bayesian credible interval).
5. **Depth:** medium (~12–16).

#### C2. **When You Don't Know σ: the t-Distribution and Small-Sample Intervals**
...
2. **Scope / inside:**
   ...
   - **Edge case:** non-normal small samples where the t-interval's coverage degrades — pointing forward to the bootstrap (E2).
3. **Assumes:** *(this track)* C1, A2. *(cross-track)* the Normal and chi-square as distributions, sums of squared normals [Probability foundations].
4. **Sets up:** D2 (the t-test is the same machinery), E2 (bootstrap as the assumption-free alternative).
5. **Depth:** deep (~18–22). **Split seam:** the chi-square/sample-variance derivation can spin out as *C2a "The Distribution of the Sample Variance (Chi-Square)"* if it crowds the t-interval.

#### C3. **Intervals From the Likelihood: Wald, Score, and Profile Intervals**
1. **Question it answers:** How do we build a confidence interval for *any* MLE-estimated parameter, not just a mean?
2. **Scope / inside:**
   - **Wald interval** `θ̂ ± z·√(I(θ̂)⁻¹)` derived from B4's asymptotic normality.
...
3. **Assumes:** *(this track)* B3, B4, C1. *(cross-track)* none new.
4. **Sets up:** D3 (these intervals are dual to the asymptotic tests); consumed directly by Regression for coefficient CIs.
5. **Depth:** medium–deep (~16–20).

### D. Hypothesis Testing

#### D1. **The Logic of Hypothesis Testing: Null, Alternative, and the Two Ways to Be Wrong**
...
3. **Assumes:** *(this track)* A1, A2, C1. *(cross-track)* conditional probability [Probability foundations].
4. **Sets up:** D2, D3, D4 — every concrete test instantiates this logic.
5. **Depth:** medium (~12–16).

#### D2. **The Workhorse Tests: z, t, χ², and F**
...
2. **Scope / inside:**
   - One- and two-sample **z- and t-tests** (the C2 machinery turned into a decision).
...
3. **Assumes:** *(this track)* C2 (t & chi-square distributions), D1. *(cross-track)* none new.
4. **Sets up:** D5 (multiple testing across many such tests), consumed by Regression (the F-test for nested models).
5. **Depth:** deep (~18–24). **Split seam:** cut into *D2a "z- and t-tests"* and *D2b "χ² and F tests,"* at the one-parameter/variance-and-counts boundary.

#### D3. **The Asymptotic Trinity: Likelihood-Ratio, Wald, and Score Tests**
...
3. **Assumes:** *(this track)* B2, B3, B4, D1, D2 (chi-square). *(cross-track)* none new.
4. **Sets up:** consumed pervasively by Regression (deviance, nested-model tests); pairs with C3 (interval duals).
5. **Depth:** deep (~22–28). **Split seam:** cut after Neyman–Pearson into *D3a "Neyman–Pearson and the Most Powerful Test"* and *D3b "The LR, Wald, and Score Trinity."*

#### D4. **Power and Sample Size: Designing a Test That Can Actually Find Something**
...
3. **Assumes:** *(this track)* D1, D2. *(cross-track)* none new.
4. **Sets up:** D5 (power under multiplicity); informs all experimental practice.
5. **Depth:** medium (~12–16).

#### D5. **When You Run Many Tests: the Multiple-Comparisons Problem**
...
3. **Assumes:** *(this track)* D1, D4. *(cross-track)* order statistics / uniform p-values under the null [Probability foundations].
...
#### E1. **The Bootstrap: Inference When You Can't Do the Math**
...
3. **Assumes:** *(this track)* A1, A2. *(cross-track)* empirical CDF, sampling with replacement [Probability foundations].
4. **Sets up:** E2 (bootstrap intervals), E3 (permutation is its testing cousin).
5. **Depth:** deep (~18–24). **Split seam:** cut after standard errors into *E1a "The Bootstrap Principle & Standard Errors"* and *E1b "When the Bootstrap Fails."*

#### E2. **Bootstrap Confidence Intervals: Percentile, Pivotal, and BCa**
...
3. **Assumes:** *(this track)* D1, E1. *(cross-track)* combinatorics of permutations [Probability foundations].
4. **Sets up:** completes the computational-inference arc; pairs with D5 (permutation-based FWER control).
5. **Depth:** medium (~12–16).

#### E4. **The Bayesian Turn: Probability as Degree of Belief**
...
3. **Assumes:** *(this track)* B1 (likelihood), C1 (to contrast intervals), D1 (to contrast testing). *(cross-track)* conditional probability & Bayes' rule for events [Probability foundations]; the Beta/Gamma distributions [Probability foundations].
...
5. **Depth:** deep (~22–28). **Split seam:** cut into *E4a "Bayes' Theorem for Parameters & Conjugate Inference"* and *E4b "Credible Intervals, MAP, and Bayesian vs. Frequentist,"* at the prior/posterior-mechanics vs. interpretation boundary.
...
```
A. FOUNDATIONS
  A1 Estimator as a Random Variable
        │
        ▼
  A2 LLN & CLT ───────────────────────────────┐
        │                                       │
...
B. LIKELIHOOD SPINE                             │
  B1 The Likelihood                             │
        │                                       │
        ▼                                       │
  B2 Maximum Likelihood (MLE)                   │
        │                                       │
        ▼                                       │
  B3 Fisher Information ──────────┐             │
        │                         │             │
        ▼                         │             │
  B4 MLE Asymptotics ◄────────────┘◄────────────┘   (uses CLT + Fisher info)
        │
        ├───────────────────────────────┐
        ▼                                ▼
C. INTERVALS                        D. TESTING
  C1 Confidence Intervals  ──┐       D1 Logic of Testing / p-values
        │                    │             │
        ▼                    │             ▼
  C2 t & χ² / small sample   │       D2 z, t, χ², F  ◄── (needs C2's distributions)
        │                    │             │
        ▼                    │             ├──► D4 Power & Sample Size
  C3 Wald/Score/Profile CI ◄─┘             │         │
        │ (B3,B4)                          ▼         ▼
        │                            D3 LR/Wald/Score Trinity   D5 Multiple Comparisons
        │  (C3 ⇄ D3 are interval/test duals)  (B2,B3,B4)
        │
        ▼
E. COMPUTATIONAL & ALTERNATIVE
  E1 The Bootstrap ──► E2 Bootstrap CIs   (C1)
        │
        └────► E3 Permutation Tests   (D1)

  E4 The Bayesian Turn   (B1, C1, D1)  ── closes the track, forks to a future MCMC track
```

Linear reading order: **A1 → A2 → B1 → B2 → B3 → B4 → C1 → C2 → C3 → D1 → D2 → D3 → D4 → D5 → E1 → E2 → E3 → E4.**
...
- **Random variables, expectation & variance** — needed at **A1**.
- **Independence & i.i.d. samples** — needed at **A1**.
- **Chebyshev's inequality + modes of convergence (in probability, in distribution)** — needed at **A2**.
- **Moment generating / characteristic functions** — needed at **A2** (CLT machinery).
- **Common distributions I: Bernoulli/Binomial, Poisson, Normal (pmf/pdf, joint density)** — needed at **B1**.
- **Common distributions II: Chi-square, Student's t, F** — needed at **C2**.
- **Common distributions III: Beta, Gamma** — needed at **E4**.
- **Conditional probability & Bayes' rule for events** — needed at **D1** and **E4**.
- **The empirical CDF / sampling with replacement** — needed at **E1**.
- **Order statistics & the uniform distribution of null p-values** — needed at **D5**.
- **Combinatorics of permutations** — needed at **E3**.

**From Geometry of Data / Linear Algebra:**
- **Gradients and the multivariable critical-point condition** — needed at **B2** (multi-parameter MLE).
- *(Light)* **Quadratic forms / the Hessian** — needed at **B3–B4** for the multi-parameter Fisher information matrix.

**From Optimization:**
- **Newton's method / gradient ascent** — needed at **B2** (when the MLE has no closed form) and **B4**.

**From Regression:** *(this track feeds Regression more than it consumes it)* — no hard prerequisite. The only inbound link is **MAP ↔ regularization** at **E4**, which is a *connection*, not a dependency.
...
|---|---|---|
| **MLE as a general estimation principle** (B1–B2) | **Inference** | It's the general engine; Regression instantiates it for β̂. |
| **Sampling distribution of an estimator** (A1) | **Inference** | The general concept; Regression specializes to β̂. |
| **Fisher information & Cramér–Rao** (B3) | **Inference** | General; Regression uses it for coefficient standard errors. |
| **MLE asymptotics / asymptotic normality** (B4) | **Inference** | Underwrites all of Regression's inference. |
| **Confidence intervals (concept + construction)** (C1, C3) | **Inference** | Regression produces intervals by applying C1/C3. |
| **Hypothesis-test logic + the F-test for nested models** (D1, D2, D3) | **Inference owns theory; Regression owns the application** | The F-test statistic/null distribution live here; Regression's "F-test for nested regression models" links back. |
| **The bootstrap** (E1–E2) | **Inference** | General method; Regression uses it for prediction/coefficient intervals. |
| **MAP ↔ ridge/lasso regularization** (E4) | **Shared, written twice on purpose at different altitudes** | Inference derives MAP-as-penalized-likelihood in general; Regression derives the specific ridge/lasso penalties. |
