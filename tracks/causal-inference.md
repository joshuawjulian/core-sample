...
```
                 [CAUS-00] Correlation Is Not Causation (and why that sentence is useless alone)
                                        │
                                        ▼
                 [CAUS-01] The Fundamental Problem of Causal Inference (the missing counterfactual)
                                        │
                                        ▼
                 [CAUS-02] Potential Outcomes: the Rubin Causal Model (Y(1), Y(0), ATE, ATT)
                                        │
                                        ▼
                 [CAUS-03] SUTVA and Positivity (the fine print that makes "an effect" well-defined)
                                        │
                  ┌─────────────────────┼─────────────────────────┐
                  ▼                                                ▼
   GRAPH LANGUAGE  [CAUS-04] Causal DAGs & the do-Operator     [CAUS-07] Randomization: the Gold Standard
                  │                                        (why an RCT identifies the ATE for free)
                  ▼                                                │
   [CAUS-05] d-Separation: Reading Independence off a Graph            │
                  │                                                │
                  ▼                                                │
   [CAUS-06] Confounders, Colliders & the Backdoor Criterion ◄─────────┘
   (+ selection bias, M-bias, "don't condition on a collider")
                  │
        ┌─────────┴───────────────── ADJUSTMENT UNDER IGNORABILITY ──────────────────┐
        ▼                                                                             ▼
   [CAUS-08] Regression Adjustment        [CAUS-09] The Propensity Score        [CAUS-10] Matching on Covariates
...
Linear reading order: **CAUS-00 → CAUS-01 → CAUS-02 → CAUS-03 → CAUS-04 → CAUS-05 → CAUS-06 → CAUS-07 → CAUS-08 → CAUS-09 → CAUS-10 → CAUS-11 → CAUS-12 →
CAUS-13 → CAUS-14 → CAUS-15 → CAUS-16 → CAUS-17.** The graph language (CAUS-04–CAUS-06) and the potential-outcomes language
(CAUS-01–CAUS-03) are two dialects for the same ideas; CAUS-06 is where they fuse (backdoor = ignorability).
...
3. **Assumes:** *(this track)* none — entry point. *(cross-track)* the idea of correlation/association [Probability foundations; Regression REGR-00].
4. **Sets up:** CAUS-01 (formalizing the missing piece), CAUS-02 (the notation), CAUS-06 (each association source gets a graph).
5. **Depth:** short–medium (~10–14). Motivational; the one article that may lean on stories over derivation.

### CAUS-01 — The Fundamental Problem of Causal Inference
...
3. **Assumes:** *(this track)* CAUS-00. *(cross-track)* expectation as a population average [Probability foundations].
4. **Sets up:** CAUS-02 (defines the averages precisely), the entire identification program.
5. **Depth:** short–medium (~10–14).

### CAUS-02 — Potential Outcomes: the Rubin Causal Model (ATE, ATT)
...
3. **Assumes:** *(this track)* CAUS-01. *(cross-track)* conditional expectation `E[Y|X]`, conditional independence [Probability foundations].
4. **Sets up:** CAUS-03 (the assumptions that make `Y(t)` well-defined), CAUS-06 (backdoor = ignorability), CAUS-08–CAUS-12 (all adjustment estimators target these estimands).
5. **Depth:** deep (~18–24). **Split seam:** cut after the estimand definitions into *CAUS-02a "Potential Outcomes and the Estimands"* and *CAUS-02b "Ignorability and the Selection-Bias Decomposition,"* at the boundary where the assumption enters.
...
2. **Scope / inside:**
   ...
   - **Positivity / overlap** `0 < P(T=1|X) < 1`: why a covariate stratum with no treated (or no control) units makes the effect *unidentifiable there*, not just imprecise — derived from CAUS-02's conditioning.
...
3. **Assumes:** *(this track)* CAUS-02. *(cross-track)* none new.
4. **Sets up:** CAUS-09/CAUS-11 (positivity is exactly what extreme propensity scores violate), CAUS-07 (RCTs guarantee positivity by design).
...
### CAUS-04 — Causal DAGs and the do-Operator
...
2. **Scope / inside:**
   ...
   - The **do-operator** `P(Y | do(X=x))` defined as graph surgery — delete the arrows *into* `X`, then read off — contrasted line by line with conditioning `P(Y | X=x)`. This is the formal "set vs. see" distinction CAUS-00 promised.
   - Structural causal models (the equations behind the graph) sketched as the generator of both the graph and the potential outcomes — the bridge that makes CAUS-02 and this article the same theory.
3. **Assumes:** *(this track)* CAUS-02 (so the do-operator can be tied to potential outcomes). *(cross-track)* joint distributions and factorization, conditional independence [Probability foundations]; directed graphs / DAG basics (built inline — it's above no floor, just unfamiliar).
4. **Sets up:** CAUS-05 (reading independences), CAUS-06 (the backdoor criterion is a do-operator theorem).
5. **Depth:** deep (~18–24). **Split seam:** cut after the Markov condition into *CAUS-04a "Causal DAGs and Structural Equations"* and *CAUS-04b "The do-Operator and Intervention,"* at the see-vs-set boundary.
...
2. **Scope / inside:**
   - The three building blocks of every path — **chain** (`A→B→C`), **fork** (`A←B→C`), **collider** (`A→B←C`) — and what flows or blocks through each, derived from the factorization in CAUS-04.
...
3. **Assumes:** *(this track)* CAUS-04. *(cross-track)* conditional independence [Probability foundations].
4. **Sets up:** CAUS-06 (backdoor and collider bias are both d-separation read-offs); gives the reader the tool to *check their own DAG*.
5. **Depth:** deep (~18–22). **Split seam:** the three path-types can spin out as *CAUS-05a "Chains, Forks, and Colliders"* if the full d-separation assembly crowds the worked graphs.

### CAUS-06 — Confounders, Colliders, and the Backdoor Criterion (with Selection Bias)
...
3. **Assumes:** *(this track)* CAUS-02 (ignorability), CAUS-04 (do-operator), CAUS-05 (d-separation). *(cross-track)* none new.
4. **Sets up:** CAUS-08–CAUS-12 (the entire adjustment family *is* backdoor adjustment, executed by different machinery); CAUS-13 (IV is what you reach for when you *can't* close the back door).
5. **Depth:** deep (~22–28). **Split seam:** cut after the backdoor criterion into *CAUS-06a "Confounding and the Backdoor Criterion"* and *CAUS-06b "Colliders, Selection Bias, and Over-Adjustment,"* at the confounder/collider boundary — they are the two opposite mistakes and each deserves room.

---

## C. The Gold Standard

### CAUS-07 — Randomized Experiments: Why an RCT Identifies the ATE for Free
...
2. **Scope / inside:**
   - Randomization makes `T ⊥ (Y(1), Y(0))` *by construction* (no `X` needed) — derived — so the naive difference in means is unbiased for the ATE; the selection-bias term from CAUS-02 is forced to zero. Guarantees positivity (CAUS-03) too.
...
3. **Assumes:** *(this track)* CAUS-02, CAUS-03. *(cross-track)* hypothesis-test logic and the permutation test [Statistical Inference INFR-D1, INFR-E3]; sampling-distribution/standard-error machinery [Statistical Inference INFR-A1]; randomized assignment as i.i.d./Bernoulli draws [Probability foundations].
...
5. **Depth:** deep (~18–24). **Split seam:** cut into *CAUS-07a "Why Randomization Identifies the ATE"* and *CAUS-07b "Analyzing an Experiment: Neyman, Fisher, and Covariate Adjustment,"* at the identification/estimation boundary.

---

## D. Adjustment Under Ignorability

*All four estimators in this section target the same backdoor-adjusted estimand (CAUS-06) under the same
ignorability assumption (CAUS-02); they differ only in **how** they implement the adjustment. CAUS-12 fuses
the two main families.*

### CAUS-08 — Regression Adjustment: the Backdoor Formula via a Model
...
2. **Scope / inside:**
   - The adjustment formula (CAUS-06) realized by modeling `E[Y | T, X]` and averaging over the covariate distribution — the **g-formula / standardization**; why the treatment coefficient in `Y ~ T + X` estimates the ATE *only* under correct functional form + ignorability + no effect modification (or with interactions, by averaging). Derived, not asserted.
   ...
   - **Reliance on the Regression track:** OLS/GLM fitting, coefficient interpretation, and standard errors are *consumed* (links to REGR-05, REGR-09, REGR-17); the new content is purely the causal *reading* of the fitted model.
3. **Assumes:** *(this track)* CAUS-02, CAUS-06. *(cross-track)* multiple regression and coefficient interpretation [Regression REGR-05, REGR-08]; GLM for non-continuous outcomes [Regression REGR-17–REGR-18]; standard errors / CIs [Statistical Inference INFR-C3].
4. **Sets up:** CAUS-12 (regression adjustment is the "outcome model" half of doubly-robust); contrast class for CAUS-09–CAUS-11.
5. **Depth:** medium–deep (~16–20).

### CAUS-09 — The Propensity Score: Collapsing Confounders to One Number
...
2. **Scope / inside:**
   ...
   - Estimating `e(X)` (usually logistic regression — *linked* to Regression REGR-17, not re-derived); checking **balance** and **overlap/common support** (positivity from CAUS-03 made operational); the difference between the *true* and *estimated* score.
...
3. **Assumes:** *(this track)* CAUS-02, CAUS-03, CAUS-06. *(cross-track)* logistic regression as the fitted probability model [Regression REGR-17]; conditional independence [Probability foundations].
...
5. **Depth:** deep (~18–22). **Split seam:** cut after the balancing theorem into *CAUS-09a "The Propensity Score and the Balancing Theorem"* and *CAUS-09b "Estimating, Checking, and the Limits of the Score,"* at the theory/practice boundary.
...
2. **Scope / inside:**
   - Matching as a direct attack on CAUS-01's missing counterfactual: impute the unseen `Y(0)` for a treated unit from its match. Exact matching → the curse of dimensionality → **distance matching** (Mahalanobis) → **propensity-score matching** (payoff from CAUS-09).
   - Design choices and their bias/variance consequences: nearest-neighbor vs. caliper, with/without replacement, k:1 ratios, trimming off-support units; why matching most naturally targets the **ATT** (CAUS-02). Assessing balance *after* matching.
...
3. **Assumes:** *(this track)* CAUS-02 (ATT), CAUS-03 (overlap), CAUS-09 (the score). *(cross-track)* distance metrics / Mahalanobis distance [Geometry of Data]; standard errors via the bootstrap [Statistical Inference INFR-E1].
...
2. **Scope / inside:**
   ...
   - The direct, visible link to positivity (CAUS-03): a propensity near 0 or 1 produces an exploding weight — *the* failure mode, made concrete; trimming and stabilization as the honest, imperfect fixes.
...
3. **Assumes:** *(this track)* CAUS-02, CAUS-03 (positivity is central here), CAUS-09 (the score). *(cross-track)* expectation of a transformed random variable, the law of total expectation [Probability foundations]; sandwich/bootstrap standard errors [Statistical Inference INFR-E1; Regression REGR-11].
...
2. **Scope / inside:**
   - The **augmented IPW (AIPW)** estimator built explicitly from CAUS-08's outcome model and CAUS-11's weights; the **double-robustness** property proved: the estimator is consistent if *either* the propensity model *or* the outcome model is correct (work the two cases separately so the reader sees each rescue).
...
3. **Assumes:** *(this track)* CAUS-08, CAUS-09, CAUS-11. *(cross-track)* the outcome model is consumed from [Regression REGR-05/REGR-17]; the weighting and SEs link to [Statistical Inference INFR-E1].
...
2. **Scope / inside:**
   - The IV conditions stated on the DAG (CAUS-04): **relevance** (`Z→T`), **exclusion** (`Z` affects `Y` only via `T`), **independence/ignorable instrument**; why these together identify an effect despite an open `T↔Y` confounding path. The **Wald estimator** `ATE = Cov(Z,Y)/Cov(Z,T)` derived; **two-stage least squares** as its regression realization (links to Regression REGR-05, not re-derived).
...
3. **Assumes:** *(this track)* CAUS-02, CAUS-06 (the open back door it routes around), CAUS-07 (noncompliance/ITT motivates it). *(cross-track)* covariance and its algebra [Probability foundations]; OLS/2SLS as the fitting engine [Regression REGR-05]; weak-instrument inference links to [Statistical Inference].
...
2. **Scope / inside:**
   - The 2×2 DiD estimator derived as a double difference (treated-vs-control × after-vs-before); the proof that it differences away any *time-invariant* group confounder — the precise sense in which it beats a cross-sectional comparison. The **parallel-trends** assumption stated as the *untestable* identifying bet, and what the counterfactual trend means in potential-outcomes terms (CAUS-02).
   - The two-way fixed-effects regression realization (links to Regression REGR-08's categorical/interaction machinery); checking pre-treatment trends as supporting (not proving) evidence; the event-study generalization sketched.
...
3. **Assumes:** *(this track)* CAUS-02, CAUS-06. *(cross-track)* fixed-effects / interaction terms in regression [Regression REGR-08]; panel-data standard errors / clustering links to [Regression REGR-11; Statistical Inference].
...
3. **Assumes:** *(this track)* CAUS-02, CAUS-13 (for the fuzzy case). *(cross-track)* local/kernel regression and bandwidth selection [Regression REGR-23–REGR-25]; continuity/limits of a function [Calc-1 floor].
...
2. **Scope / inside:**
   - The premise: ignorability (CAUS-02) and exclusion (CAUS-13) can never be verified from data, so the honest deliverable is not a point estimate but **how fragile that estimate is**. Reframes the whole observational program as "assume, then stress-test."
...
3. **Assumes:** *(this track)* CAUS-02, CAUS-06, CAUS-09/CAUS-10/CAUS-11 (the estimates being stressed), CAUS-13 (exclusion). *(cross-track)* p-values / significance [Statistical Inference INFR-D1]; odds/risk ratios [Regression REGR-17].
...
3. **Assumes:** *(this track)* all of CAUS-00–CAUS-16.
4. **Sets up:** Terminal node. Hands off to future tracks: **ML for causal inference** (double ML, causal forests, TMLE — seeded at CAUS-12), **mediation / direct & indirect effects** (seeded at CAUS-06), **time-varying treatments / marginal structural models** (seeded at CAUS-11), and **synthetic control** (seeded at CAUS-14).
...
- **Conditional expectation `E[Y|X]` and conditional independence** — needed at **CAUS-02** (and pervasively; this is the language of ignorability).
- **Joint distributions, factorization, the law of total expectation** — needed at **CAUS-04, CAUS-11**.
...
- **Bernoulli/binomial assignment, i.i.d. sampling** — needed at **CAUS-07**.

**From the Regression track:**
- **Multiple regression, coefficient interpretation ("holding others fixed"), categorical & interaction terms** — needed at **CAUS-08, CAUS-14**.
- **Logistic regression as a fitted probability model** — needed at **CAUS-09** (propensity score), **CAUS-16** (odds ratios).
- **GLMs** for non-continuous outcomes — needed at **CAUS-08**.
...
- **Sampling distribution / standard error of an estimator (INFR-A1)** — needed wherever an estimate gets a CI (**CAUS-07 onward**).
- **Hypothesis-test logic and p-values (INFR-D1)** — needed at **CAUS-07, CAUS-16**.
- **The permutation/randomization test (INFR-E3)** — needed at **CAUS-07** (Fisher's exact test of the sharp null).
- **The bootstrap (INFR-E1)** — needed at **CAUS-10, CAUS-11, CAUS-12** (standard errors with no closed form).
- **Confidence-interval construction (INFR-C1, INFR-C3)** — needed at **CAUS-08 onward**.
...
| **OLS / GLM / 2SLS fitting and coefficient mechanics** | **Regression** | This track *reads* a fitted coefficient causally (CAUS-08, CAUS-13); it never re-derives the fit. |
...
| **Logistic regression (the propensity model itself)** | **Regression (REGR-17)** | Causal Inference owns the propensity score's *causal role and balancing theorem* (CAUS-09); the logistic fit is Regression's. |
...
| **Simpson's paradox / confounding *intuition*** | **shared with Probability & Regression, owned here for the *causal* resolution** | Other tracks may show the arithmetic; this track owns *why* it resolves one way or the other (it's a causal, not statistical, question — CAUS-00). |

**Versus the tracks being designed concurrently:**
- **Probability Foundations** — supplies conditional independence, joint factorization, expectation algebra (the language CAUS-02–CAUS-05 are written *in*). Pure consumer relationship; no overlap, only dependency.
...
- **Classification** — overlaps **only** at the propensity score: a propensity model *is* a probabilistic classifier. Recommendation: **Classification owns the classifier; Causal Inference owns its use as a balancing score (CAUS-09).** The two should cross-link, not duplicate. Classification's calibration content is directly relevant to good propensity estimates and should be referenced from CAUS-09.
